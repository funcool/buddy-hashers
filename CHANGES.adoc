...
- The `bcrypt+sha386` algorithm is becomes the new default password hasher. <br />
  ...
  does not uses any explicit hash truncation.<br />
...
- Fixed wrong output digest size in `pbkdf2+sha256`. <br />
  ...
  `pbkdf2+sha256b$` if you want continue validating properly that passwords. <br />
